...
export async function getKoalaPunctuation(message) {
  const openaiResponse = await openaiRequest({
    ...
    "prompt": "Koala is a friendly chatbot that is going to analyze OBJECTIVELY in a scale from"
      + " 0 (incomprehensible sentence) to 10 (regular written english)"
      + " how well my english grammar and spelling are after the following phrase: " + message
      + "\n\nscore (number) and recommendations: ",
